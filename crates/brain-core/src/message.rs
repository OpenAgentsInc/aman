...
pub enum TaskHint {
    ...
    /// Questions about the bot itself (capabilities, identity, how it works).
    AboutBot,
}
...
/// A text style range for message formatting.
#[derive(Debug, Clone, Default)]
pub struct TextStyle {
    ...
    pub length: u32,
    /// Style type (BOLD, ITALIC, MONOSPACE, STRIKETHROUGH).
    pub style: String,
}

impl TextStyle {
    /// Create a new text style.
    pub fn new(start: u32, length: u32, style: impl Into<String>) -> Self {
        Self {
            start,
            length,
            style: style.into(),
...
pub struct OutboundMessage {
    ...
    /// Optional text styles for formatting.
...
impl OutboundMessage {
    ...
    /// Check if this message has any formatting styles.
    pub fn has_styles(&self) -> bool {
        !self.styles.is_empty()
    }

    /// Add text styles to this message.
